...
RUN go build -o /go/bin/persona-service -trimpath -ldflags="-w -s" github.com/linuxfoundation/lfx-v2-persona-service/cmd/server
...
COPY --from=builder /go/bin/persona-service /cmd/persona-service

ENTRYPOINT ["/cmd/persona-service"]
